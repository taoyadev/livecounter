// YouTube Search Video API Route
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${SOCIAL_API_BASE_URL}/youtube/search/video?query=${encodeURIComponent(query)}`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('YouTube Search Video API Error:', error)
    return NextResponse.json(
